...
char directionTravelingLeft;
long int clickRotationsLeft;
char directionTravelingRight;
long int clickRotationsRight;
char directionTravelingLauncher;
long int clickRotationsLauncher;
char oldValueLeft;
char oldValueRight;
char oldValueLauncher;

int clicksTillIncrease;
...
void Initialize_Interrupts(void)  
{
	...
	oldValueRight = 0;
	oldValueLauncher = 0;
	directionTravelingLeft = 0;
	clickRotationsLeft = 0;
	directionTravelingRight = 0;
	clickRotationsRight = 0;
	directionTravelingLauncher = 0;
	clickRotationsLauncher = 0;
	clicksTillIncrease = 0;
...
{
	if(RIGHTENC2 == INPUTTRUE){
		clickRotationsRight++;
		directionTravelingRight = FORWARD;
	}else{
		clickRotationsRight--;
		directionTravelingRight = BACKWARD;
	}
}
...
{	
	if(LEFTENC2 == INPUTTRUE){
		clickRotationsLeft--;
		directionTravelingLeft = FORWARD;
	}else{
		clickRotationsLeft++;
		directionTravelingLeft = BACKWARD;
	}
...
void Int_4_Handler(unsigned char RB5_State)
{
	...
	if(RB5_State == INPUTTRUE){
		if(LAUNCHENC2 == INPUTTRUE){
			if(clicksTillIncrease >= 10){
				directionTravelingLauncher = FORWARD;
				clickRotationsLauncher--;
				clicksTillIncrease = 0;
			}else{
				clicksTillIncrease++;
			}
			...
			if(clicksTillIncrease <= -10){
				directionTravelingLauncher = BACKWARD;
				clickRotationsLauncher++;
				clicksTillIncrease = 0;
			}else{
				clicksTillIncrease--;
	...
	/*Relay2_red = 1;
	if(RB5_State == INPUTTRUE){
		oldValueLauncher = LAUNCHENC2;
    }else{
	    if(LAUNCHENC2 == INPUTTRUE){
		    if(oldValueLauncher == INPUTTRUE){
				//ERROR BAD CLICK
			}else{
				//good click
				if(clicksTillIncrease <= -7) {
           			directionTravelingLauncher = FORWARD;
           			clickRotationsLauncher++;
           			clicksTillIncrease = 0;
           		}else{
           ...
           if(oldValueLauncher == INPUTFALSE){
				//ERROR BAD CLICK
			}else{
				//good click
				if(clicksTillIncrease >= 7){
