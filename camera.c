...
char light_centered_p(void)
{
  ...
  if(T_Packet_Data.mx != 0)
  {
    ...
    t_tilt_error = (int)T_Packet_Data.my - TILT_TARGET_PIXEL_DEFAULT;
    if (t_tilt_error < 0)
    {
      t_tilt_error = (-1 * t_tilt_error);
    }
    if (t_tilt_error < TILT_ALLOWABLE_ERROR_DEFAULT && t_pan_error < PAN_ALLOWABLE_ERROR_DEFAULT)
    {
      return YES;
    }
  }
...
void set_current_camera(int num)
{
  if (num == CAMERA_ONE)
  {
    ...
    current_camera_serial_port = CAMERA_SERIAL_PORT_2;
    current_terminal_serial_port = TERMINAL_SERIAL_PORT_1;
    stdout_serial_port = SERIAL_PORT_ONE;
  }
  else if (num == CAMERA_TWO)
  {
    current_camera = CAMERA_TWO;
    current_camera_serial_port = CAMERA_SERIAL_PORT_1;
    current_terminal_serial_port = TERMINAL_SERIAL_PORT_2;
    stdout_serial_port = SERIAL_PORT_TWO;
	if (camera_two_initialized == 0)
	{
      camera_two_initialized = 1;
